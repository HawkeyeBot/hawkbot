...
class Last24hVolumeFilter(Filter):
    @classmethod
    def filter_name(cls):
        return cls.__name__
    ...
    def filter_symbols(self,
        ...
        ordered_volume = collections.OrderedDict()
        changes: Dict[str, ChangeStatistic] = self.exchange.fetch_last_24h_changes()
        for symbol in starting_list:
            volume_change = changes[symbol].quote_volume
            ordered_volume[volume_change] = symbol

        if self.sort is not None:
            reverse = True if self.sort == 'desc' else False
            ordered_volume = sorted(ordered_volume.items(), reverse=reverse)

        selected_list = [row for volume, row in ordered_volume]
        if self.top:
