...
class DynamicCatcherEquilibriumShortStrategy(AbstractBaseStrategy):
    def __init__(self):
        super().__init__()
        ...
        self.global_short_min_distance_pct: float = None
    ...
    def init_config(self):
        ...
        if self.global_short_min_distance_pct is None:
            raise InvalidConfigurationException("Parameter 'global_short_min_distance_pct' is required.")
    ...
    def on_no_open_position(self,
        ...
        # check short distance conditions
        allow_short = current_price >= last_close * (1 + self.global_short_min_distance_pct / 100)
        if not allow_short:
            logger.info(f'{symbol}: Skipping action as allow_short condition is not met.')
            return
