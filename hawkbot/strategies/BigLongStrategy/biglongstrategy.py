...
class BigLongStrategy(AbstractBaseStrategy):
    def __init__(self):
        super().__init__()
    ...
    def block_because_spike_detected(self,
        ...
        if current_price >= lowest_candle_high * (1 + self.spike_detection_block_entry_threshold):
            logger.info(f"{symbol} {self.position_side.name}: BLOCKING entry because current price {current_price} is more than "
            ...
            logger.info(f"{symbol} {self.position_side.name}: ALLOWING entry because current price {current_price} is less than "
                        f"{readable_pct(self.spike_detection_block_entry_threshold, 2)} from last {self.spike_detection_nr_candles} candles' lowest high "
                        f"{lowest_candle_high}")
            return False
