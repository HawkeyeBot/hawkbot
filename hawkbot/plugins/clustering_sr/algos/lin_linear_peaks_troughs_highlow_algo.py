...
class LinLinearPeaksTroughsHighLowAlgo(Algo):
    """
    ...
    def __init__(self, algo_config: Dict = None):
        ...
        self.linear_algo = LinearAlgo()
        self.pt = PeaksTroughsHighLowAlgo()
        self.nr_clusters_from_linspace: int = 2
    ...
    def calculate_levels(self,
        ...
        lin_support_resistances = self.lin_algo.calculate_levels(symbol,
                                                                 position_side,
                                                                 candles,
                                                                 nr_clusters,
                                                                 current_price,
                                                                 ...
                                                                 original_start_date,
                                                                 symbol_information)
        filtered_lin_support_resistances = SupportResistance(supports=lin_support_resistances.supports[:self.nr_clusters_from_linspace],
                                                             resistances=lin_support_resistances.resistances[:self.nr_clusters_from_linspace])
        all_support_resistances = self.add_linear_levels(symbol=symbol,
                                                         ...
                                                         filtered_lin_support_resistances=filtered_lin_support_resistances)
        ...
        for s in all_support_resistances.supports:
        ...
        for r in all_support_resistances.resistances:
    ...
    def add_linear_levels(self,
                          ...
                          filtered_lin_support_resistances: SupportResistance):
        supports = filtered_lin_support_resistances.supports
        resistances = filtered_lin_support_resistances.resistances
        if position_side == PositionSide.LONG:
            linear_clusters = self.linear_algo.calculate_levels(symbol=symbol,
                                                                position_side=position_side,
                                                                candles=candles,
                                                                nr_clusters=nr_clusters - self.nr_clusters_from_linspace,
                                                                current_price=min(supports),
                                                                outer_price=outer_price,
                                                                original_start_date=original_start_date,
                                                                symbol_information=symbol_information)
            supports.extend(linear_clusters.supports)
        else:
            linear_clusters = self.linear_algo.calculate_levels(symbol=symbol,
                                                                position_side=position_side,
                                                                candles=candles,
                                                                nr_clusters=nr_clusters - self.nr_clusters_from_linspace,
                                                                current_price=min(resistances),
                                                                outer_price=outer_price,
                                                                original_start_date=original_start_date,
                                                                symbol_information=symbol_information)
            resistances.extend(linear_clusters.resistances)
