...
class BasicLongStrategy(AbstractBaseStrategy):
    def __init__(self):
        super().__init__()
    ...
    def on_no_open_position(self,
        ...
        if not self.specified_supports_available_for_all_timeframes(symbol=symbol,
                                                                    position_side=position_side,
                                                                    current_price=current_price):
            if self.dca_config.override_insufficient_levels_available:
                user_log.info(f"{symbol} {position_side.name}: Entering a position while there are insufficient "
                              f"support prices available to meet the configuration's minimum number of supports, but "
                              f"the configuration option 'override_insufficient_supports_available' is set to true",
                              __name__)
            else:
                return
    ...
    def specified_supports_available_for_all_timeframes(self,
                                                        symbol: str,
                                                        position_side: PositionSide,
                                                        current_price: float):
        # check the minimum number of supports specified
        minimum_number_supports = self.minimum_number_of_available_dcas

        # calculate all the support prices
        number_available_support_prices = len(
            self.dca_plugin.calculate_support_prices(symbol=symbol,
                                                     position_side=position_side,
                                                     maximum_price=current_price,
                                                     dca_config=self.dca_config))

        if number_available_support_prices < minimum_number_supports:
            if not self.dca_config.override_insufficient_levels_available:
                logger.warning(f"{symbol} {position_side.name}: Based on the history of {symbol}, there are "
                               f"{number_available_support_prices} supports, which is less than the minimum number of "
                               f"supports ({minimum_number_supports}) specified in the config. If you want the bot to "
                               f"enter a position despite this warning, please set the field "
                               f"'override_insufficient_level_available' to true in the configuration file, or "
            ...
            logger.debug(f'{symbol} {position_side.name}: Based on the history, there are '
                         f'{number_available_support_prices}, which is at least the minimum number of supports '
                         f'({minimum_number_supports}) specified in the config')
            return True
    ...
    def create_initial_entry_order(self,
        ...
        highest_bid = self.orderbook.get_highest_bid(symbol=symbol, current_price=current_price)
        min_entry_qty = calc_min_qty(price=highest_bid,
                                     inverse=False,
                                     qty_step=symbol_information.quantity_step,
                                     min_qty=symbol_information.minimum_quantity,
                                     min_cost=symbol_information.minimal_buy_cost)
        max_entry_qty = round_(cost_to_quantity(cost=wallet_balance * wallet_exposure * self.initial_cost,
                                                price=highest_bid, inverse=False),
                               step=symbol_information.quantity_step)
    ...
    def on_tp_refill_order_filled(self,
        ...
        if used_root_price is not None and rounded_position_price > rounded_root_price:
            user_log.info(f"{symbol} {position_side.name}: Updating DCA grid because the current position price "
                          f"{rounded_position_price} is > than the used root price of {rounded_root_price}")
            self.dca_plugin.initialize_unlimited_grid(symbol=symbol,
                                                      position_side=position_side,
                                                      symbol_information=symbol_information,
        ...
        if self.enforce_tp_grid(position=position,
                                ...
                                current_price=current_price,
                                wiggle_config=self.wiggle_config):
            user_log.info(f"{symbol} {position_side.name}: Recreating TP grid because TP_REFILL "
                          f"order was filled", __name__)
