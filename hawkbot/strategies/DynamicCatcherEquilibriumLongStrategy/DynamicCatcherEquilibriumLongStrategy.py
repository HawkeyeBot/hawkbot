...
class DynamicCatcherEquilibriumLongStrategy(AbstractBaseStrategy):
    def __init__(self):
        super().__init__()
    ...
    def on_no_open_position(self,
        ...
        # retrieve the filter result for the symbol to check the 'long' flag from redis
        filter_result = self.get_filter_result_for_symbol(symbol)

        if filter_result is not None and not filter_result.get('long', False):
            logger.info(f'{symbol} {position_side.name}: Skipping action because long is False for this symbol')
            return
